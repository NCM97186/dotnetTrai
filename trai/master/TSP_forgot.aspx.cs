...
public partial class TSP_forgot : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (flag == 0)
            {
                if (str1 != "")
                {
                    com = new MySqlCommand("select count(*) from TRAI_TSPUsers where(uname='" + str1 + "')", con);
                    con.Open();
                ...
                {
                    com = new MySqlCommand("select count(*) from TRAI_TSPUsers where(email='" + str2 + "')", con);
                    con.Open();
                    dr = com.ExecuteReader();
                    dr.Read();
                    cntr = Convert.ToInt32(dr[0].ToString().Trim());
            ...
            if (flag == 0)
            {
                ...
                if (str1 != "")
                {
                    com = new MySqlCommand("select * from TRAI_TSPUsers where(uname='" + str1 + "')", con);
                }
                else
                {
                    com = new MySqlCommand("select * from TRAI_TSPUsers where(email='" + str2 + "')", con);
                }
                con.Open();
                dr = com.ExecuteReader();
                dr.Read();
