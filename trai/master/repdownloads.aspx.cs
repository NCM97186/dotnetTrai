...
public partial class repdownloads : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        tablename = "TRAI_downloadcounter";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (flag == 0)
            {
                ...
                mystr = mystr + "<tr><td class=tablehead align=center><b>Date</b></td><td class=tablehead align=center><b>Excel Downloads</b></td><td class=tablehead align=center><b>XML Downloads</b></td></tr>";
                ...
                while(tempdate<dt2)
                {
                    double excelcount=0;
                    double xmlcount=0;
                    com = new MySqlCommand("select count(*) from " + tablename + " where(recdate>='" + tempdate.ToString("yyyy-MM-dd") + "' and recdate<'" + tempdate.AddDays(1).ToString("yyyy-MM-dd") + "') and (dtype='Excel')", con);
                    con.Open();
                    dr = com.ExecuteReader();
                    dr.Read();
                    try
                    {
                        excelcount = Convert.ToDouble(dr[0].ToString().Trim());
                    }
                    catch (Exception ex) { }
                    con.Close();
                    com = new MySqlCommand("select count(*) from " + tablename + " where(recdate>='" + tempdate.ToString("yyyy-MM-dd") + "' and recdate<'" + tempdate.AddDays(1).ToString("yyyy-MM-dd") + "') and (dtype='XML')", con);
                    con.Open();
                    dr = com.ExecuteReader();
                    dr.Read();
                    try
                    {
                        xmlcount = Convert.ToDouble(dr[0].ToString().Trim());
                    }
                    catch (Exception ex) { }
                    con.Close();
                    ...
                    mystr = mystr + "<td class=" + css + " align=center valign=top>" + excelcount.ToString().Trim() + "</td>";
                    mystr = mystr + "<td class=" + css + " align=center valign=top>" + xmlcount.ToString().Trim() + "</td>";
                    mystr = mystr + "</tr>";
